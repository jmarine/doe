...
public class JPAUtils 
{
    ...
    public static Object createEntity(String puName, Object entity)
    {
        if(entity != null) {
            EntityManager manager = org.doe4ejb3.util.JPAUtils.getEntityManager(puName);
    ...
    {
        // this method also create new entities, but may replace existing entities with same @Id 
        // (it may be dangerous when @Id values aren't autogenerated).
        if(entity != null) {
            EntityManager manager = org.doe4ejb3.util.JPAUtils.getEntityManager(puName);
            EntityTransaction transaction = manager.getTransaction();
            transaction.begin();
